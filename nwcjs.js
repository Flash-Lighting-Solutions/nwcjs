...
var nwcjs = {
  ...
  sha256: async (text_or_bytes) => {
    if (typeof text_or_bytes === "string")
      text_or_bytes = new TextEncoder().encode(text_or_bytes);
  ...
  processNWCstring: (string) => {
    if (!string.startsWith("nostr+walletconnect://"))
      return alert(
        `Your pairing string was invalid, try one that starts with this: nostr+walletconnect://`
      );
    ...
    obj["app_pubkey"] = nobleSecp256k1
      .getPublicKey(obj["app_privkey"], true)
      .substring(2);
  ...
  makeLiquidAddress: async (nwc_info, seconds_of_delay_tolerable = 3) => {
    var msg = JSON.stringify({
      method: "make_liquid_address",
      params: {
        asset_id:
    ...
    var loop = async () => {
      ...
      var one_i_want = null;
      nwcjs.response.every((item, index) => {
        if (item["result_type"] === "make_liquid_address") {
          one_i_want = item;
          nwcjs.response.splice(index, 1);
          return;
        }
  ...
  didPaymentSucceed: async (
    nwc_info,
    invoice,
    seconds_of_delay_tolerable = 3
  ) => {
    var invoice_info = await nwcjs.checkInvoice(
      nwc_info,
      invoice,
      seconds_of_delay_tolerable
    );
    if (
      invoice_info &&
      "result" in invoice_info &&
      "preimage" in invoice_info["result"] &&
      invoice_info["result"]["preimage"]
    )
  ...
  tryToPayLiquid: async (nwc_info, address, amount) => {
    var msg = {
      method: "pay_liquid",
      params: {
        address,
  ...
  getZapRequest: async (lnaddy, amount, relays = ["wss://nostrue.com"]) => {
    ...
    var lnurl = bech32.bech32.encode(
      "lnurl",
      bech32.bech32.toWords(url_bytes),
      100_000
    );
